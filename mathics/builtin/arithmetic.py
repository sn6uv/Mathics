...
class _PlusTimes(SympyFunction):
    def apply(self, items, evaluation):
        ...
        sym_method = getattr(sympy, self.sympy_name)
        sym_expr = sym_method(*sym_items)
        result = from_sympy(sym_expr)

        if raw_items:
            if result.has_form(self.get_name(), None):
                result.leaves.extend(raw_items)
            else:
                result = Expression(self.get_name(), result, *raw_items)
        return result


class Plus(BinaryOperator, _PlusTimes):
    """
    <dl>
    <dt>'Plus[$a$, $b$, ...]'</dt>
    <dt>$a$ + $b$ + ...</dt>
...
class Times(BinaryOperator, _PlusTimes):
    """
    <dl>
    <dt>'Times[$a$, $b$, ...]'</dt>
    <dt>'$a$ * $b$ * ...'</dt>
